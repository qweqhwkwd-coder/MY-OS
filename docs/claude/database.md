...
в `user_stats` — этими тремя колонками активно пользуется `backend/db.py`, значит миграция была
применена к боевой БД, но файл потерян/не закоммичен. Сверить фактическую схему `user_stats`
в Supabase и при возможности восстановить файл `002_add_stats.sql` задним числом для истории.
...
## Таблицы (001 + 003–014, + предположительно 002)
